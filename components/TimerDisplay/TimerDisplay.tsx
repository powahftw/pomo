...
export default function TimerDisplay({ secondsLeft, currStage }) {
  const secondstoHHMMSS = (sec: number) => {
    const h = sec < 0 ? 0 : Math.floor(sec / (60 * 60));
    const m = sec < 0 ? 0 : Math.floor(sec / 60);
    const s = sec < 0 ? 0 : sec % 60;
    return (
      [h, m, s]
  ...
  const currBgColor = currStage === Stage.WORK ? "bg-blue-100" : "bg-green-100";
  const currColor =
    currStage === Stage.WORK ? "text-blue-500" : "text-green-500";
